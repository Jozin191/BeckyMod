...
---@param tab2 any[]
---@function
function BeckyMod:AppendTable(tab1, tab2)
...
---@function
function BeckyMod:CombineTables(...)
	local tabs = { ... }
	local out = {}
...
---@function
function BeckyMod:AddToDictionary(dict1, dict2)
...
---@return V[]
---@function
function BeckyMod:GetValues(tab)
	local out = {}
...
---@function
function BeckyMod:PrintTable(node)
...
---@return integer
---@function
function BeckyMod:TableLength(tab)
	local count = 0
...
---@return boolean
---@function
function BeckyMod:ContainsValue(tab, val)
	for _, v in pairs(tab) do
		if v == val then
			return true
		end
...
---@return table<K,V>
---@function
function BeckyMod:ShuffleTable(tab, rng)
	if not rng then
		rng = Mod.GENERIC_RNG
	end
	local out = BeckyMod:CopyTable(tab)
...
---@return table<K,V>
---@function
function BeckyMod:ClearTable(tab, isArray)
	if isArray then
		local count = #tab
		for i = 1, count do
			tab[i] = nil
...
---@function
function BeckyMod:FlattenTable(tab, depth, out)
	out = out or {}
	for _, v in pairs(tab) do
		if type(v) == "table" and (not depth or depth > 0) then
			Mod:FlattenTable(v, depth and depth - 1, out)
		else
			table.insert(out, v)
		end
	end
...
---@function
function BeckyMod:MaxInTable(tab, comparer)
...
---@function
function BeckyMod:MinInTable(tab, comparer)
	comparer = comparer or function(a, b)
		return a < b
	end
...
---@function
function BeckyMod:GetValueFromWeightedPool(pool, rng)
	...
	while roll > 0 and index < #pool do
		...
		--Mod:DebugLog("Index is:", index)
		--Mod:DebugLog("Roll is:", roll)
	end

	-- V is the value of the selected pool item
	--Mod:DebugLog("Available Pools: ")
	for num, value in ipairs(pool) do
		local weight = GetWeight(value)
		if weight == math.floor(weight) then
			weight = math.floor(weight)
		end
		--Mod:DebugLog(num .. " With weight " .. weight)
	end
	if index == 0 then
		--Mod:DebugLog("Pool exhausted. Returned nil")
		return nil, index
	end
	--Mod:DebugLog("Chosen Pool:" .. index)
...
---@function
function BeckyMod:GetValuesByKeys(tab, keys)
	local out = {}
...
---@function
function BeckyMod:Map(tab, func)
	local out = {}
...
---@return table<K,V>
---@function
function BeckyMod:FilterDict(dict, filterFunc)
	local out = {}
...
---@return V[]
---@function
function BeckyMod:FilterList(list, filterFunc)
	local out = {}
...
---@return K?, V?
function FindInTable(tab, checker)
...
---@param ... any
---@return T[]
function RepeatFill(count, funcOrVal, ...)
	local out = {}
...
---@param tab table<K,V>
---@return table<K,V>
function BeckyMod:ShallowCopy(tab)
	local out = {}
	for k, v in pairs(tab) do
		out[k] = v
	end
...
---@param tab V
---@return V
function BeckyMod:CopyTable(tab)
	return Mod.SaveManager.Utility.DeepCopy(tab)
end

---@generic V
---@param dict table<any, V>
---@return V[]
function BeckyMod:ToList(dict)
	local out = {}
