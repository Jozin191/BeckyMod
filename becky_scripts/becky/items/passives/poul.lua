...
local function ShootGrid(fam, vel)
    ...
    if #grids == 1 then
        ...
        for i, grid in ipairs(grids) do
            local gridTypeVar= grid:GetData().GridTypeVar
            local tear = Isaac.Spawn(2, TearVariant.GRIDENT, (gridTypeVar or 0), famPos, vel:Rotated( (rng:RandomInt(24) - 12) * i), grid):ToTear()
...
BeckyMod:AddCallback(ModCallbacks.MC_FAMILIAR_UPDATE, function(_, fam)
    ...
    if state == FAM_STATES.SELECT_GRID then
        ...
        if gridIdx < 0 then
            if Isaac.CountEnemies() ==0 then
                if #TaintedRocksList >0 then
                    ...
                    fam.Coins = TaintedRocksList[ tableIdx ]
            ...
            if fam.Coins < 0 then fam.State = FAM_STATES.ATTACKING
            elseif player:HasCollectible(CollectibleType.COLLECTIBLE_BFFS) then
                local fromPos = room:GetGridPosition(fam.Coins)
                local grids = {}
                for i=-180, 135, 45 do
                ...
                if #grids >0 then
                    fam.Keys = grids[rng:RandomInt(1, #grids)]
                end
            end
            return
        end
        ...
        if not room:CanPickupGridEntity(gridIdx) then
            if player:HasCollectible(CollectibleType.COLLECTIBLE_BFFS) and famData.Grids and #famData.Grids > 0 then
    ...
    elseif state == FAM_STATES.PICKING_GRID then
        ...
        if sp:IsFinished() then
            if famData.Grids and #famData.Grids == 0 then
                ...
                if player:HasCollectible(CollectibleType.COLLECTIBLE_BFFS) and fam.Keys >= 0 then
                    fam.Coins = fam.Keys
                    fam.Keys = -1
    ...
    elseif state == FAM_STATES.HOLDING_GRID or state == FAM_STATES.ATTACKING then
        ...
        if target == nil or target:IsDead() or not target:Exists() then
            fam.Target = nil
            if state == FAM_STATES.HOLDING_GRID then
                ShootGrid(fam, Vector.Zero)
                fam:GetSprite():Play("Idle", true)
            end
            if Isaac.CountEnemies() > 1 then
                fam.State = FAM_STATES.FOLLOW_PLAYER
            else fam.State = FAM_STATES.IDLE end
            return
        end
...
BeckyMod:AddCallback(ModCallbacks.MC_POST_GRID_ENTITY_SPAWN, function(_, gridEnt)
    ...
    if VALID_GRID_TYPES[gridType] then
        ...
        if gridType == GridEntityType.GRID_ROCKT or gridType == GridEntityType.GRID_ROCK_ALT2 then
            table.insert(TaintedRocksList, idx)
            TaintedRocksList.IsTaintedRockGrid = TaintedRocksList.IsTaintedRockGrid or {}
            TaintedRocksList.IsTaintedRockGrid[idx] = #TaintedRocksList
        end
