...
BeckyMod:AddCallback(BeckyMod.Callbacks.ON_GHOST_HIT_ENEMY, function(_, fam, enemy, tearParams)
    ...
    if tearParams.TearFlags & TearFlags.TEAR_SPLIT == TearFlags.TEAR_SPLIT then
        local rot = (enemy.Position-fam.Position):GetAngleDegrees()+90
        for i = 1, 2 do
            local tear = player:FireTear(fam.Position, Vector(1, 0):Rotated((i/2)*360+rot)*player.ShotSpeed * 10, false, true, false, fam)
    ...
    if tearParams.TearFlags & TearFlags.TEAR_QUADSPLIT == TearFlags.TEAR_QUADSPLIT then
        ...
        for i = 1, 4 do
            local tear = player:FireTear(fam.Position, Vector(1, 0):Rotated((i/4)*360+rot)*player.ShotSpeed * 10, false, true, false, fam)
    ...
    if tearParams.TearFlags & TearFlags.TEAR_BONE == TearFlags.TEAR_BONE then
        ...
        for i = 1, rng:RandomInt(1,3) do
            local tear = player:FireTear(fam.Position, RandomVector()*player.ShotSpeed * 10, false, true, false, fam)
            tear.CollisionDamage = tear.CollisionDamage*.3
            tear.Scale = 1
