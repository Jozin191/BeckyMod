...
local function peppersUpdate(_, familiar)
    ...
    if player:HasCollectible(CollectibleType.COLLECTIBLE_GHOST_PEPPER) then
        ...
        if rng:RandomInt(100) + 1 <= 1 then
            local fire = BeckyMod.Game:Spawn(EntityType.ENTITY_EFFECT, EffectVariant.BLUE_FLAME, familiar.Position, familiar.Velocity, familiar, 0, 1):ToEffect()
            fire.Timeout = 45
    ...
    if player:HasCollectible(CollectibleType.COLLECTIBLE_BIRDS_EYE) then
        ...
        if rng:RandomInt(100) + 1 <= 1 then
            local fire = BeckyMod.Game:Spawn(EntityType.ENTITY_EFFECT, EffectVariant.RED_CANDLE_FLAME, familiar.Position, familiar.Velocity, familiar, 0, 1):ToEffect()
            fire.Timeout = 45
            fire.CollisionDamage = 14
