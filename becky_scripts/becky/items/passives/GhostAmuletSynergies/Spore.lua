...
BeckyMod:AddCallback(Callbacks.ON_GHOST_HIT_ENEMY, function(_, fam, enemy)
    ...
    if not player:HasCollectible(CollectibleType.COLLECTIBLE_MUCORMYCOSIS) then return end

    local rng = player:GetCollectibleRNG(CollectibleType.COLLECTIBLE_MUCORMYCOSIS)

    if rng:RandomFloat() > 0.25 then return end

    local tear = Isaac.Spawn(EntityType.ENTITY_TEAR, TearVariant.SPORE, 0, enemy.Position, Vector.Zero, player):ToTear() ---@cast tear EntityTear
